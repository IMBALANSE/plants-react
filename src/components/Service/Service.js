...
/* - useState: Используется для хранения активных фильтров. activeFilters — это массив, который содержит текущие выбранные фильтры.
    - useRef: Используется для хранения ссылок на DOM-элементы (кнопки фильтров и карточки). 
    filterBtnsRef и cardsRef — это массивы, которые хранят ссылки на соответствующие элементы.
...
//Описание функции handleFilterClick
      ...
      setActiveFilters(prevFilters => { // prevFilters это текущее значение состояния activeFilters на момент вызова функции обновления setActiveFilters.
        itemBtn.classList.remove(styles.activeBtn); // уберем CSS свойство активной кнопки. При первом нажатии этого свойства нет ни у кого , его можно будет убрать только при втором нажатии
        if (prevFilters.includes(filterDataValue)) { // через .includes проверим, есть ли filterDataValue в массиве. // вернет булевое.// при первом клике false при втором тру
          ...
          if (prevFilters.length < 2) {
            itemBtn.classList.add(styles.activeBtn); //Добавим CSS класс у текущей кнопки фильтра
            ...
            document.querySelector(`.${styles.service__button}[data-name="${removedFilter}"]`).classList.remove(styles.activeBtn); //удаляем его класс
            itemBtn.classList.add(styles.activeBtn); //Добавим CSS класс у текущей кнопки фильтра
  ...
  useEffect(() => {
    const updateCards = () => {
      cardsRef.current.forEach(card => { // Перебираем карточки
        const cardCategories = card.dataset.name.split(' '); // создадим из массива элементов массивы с одним элементом внутри через .split(' ') Т.е. из ['1', '2', '3'] получим  ['1'] ['2'] ['3']
        if (activeFilters.some(filter => cardCategories.includes(filter))) { // Если хотя бы один активный фильтр совпадает с категорией карточки, убираем blur
          card.classList.remove(styles.blur); // убираем стиль блюр у текущей карточки
  ...
  return (
    <section className={styles.service} id="service">
      <div className={styles.service__wrapper} >
        <div className= {`${styles.service__header} flex__row` }>
          <h2>Service and our projects</h2>
          <div className={`${styles.service__buttons } flex__row` }>
        ...
        <div className={`${styles.service__body} flex__row` }>
          <div className={`${styles.service__item} flex__column` } data-name="garden">
          ...
          <div className={`${styles.service__item} flex__column` } data-name="planting">
            <picture className={styles.service__item__image}>
          ...
          <div className={`${styles.service__item} flex__column` } data-name="lawn">
            <picture className={styles.service__item__image}>
