        ...
        <div className={`${styles.prices__content} ` } >
          ...
          <div className={`${styles.prices__list} ` } >
            {accordionData.map((item, index) => {
              ...
              return (
                <div key={index} className={`${styles.accordion__item} ${activeClass}`}> {/*start если активный элем совпадает с индексом, то присовим этому элем. класс active */}
                  <div className={`${styles.accordion__header} ${activeClass}`} onClick={() => toggleAccordion(index)}> {/*28,03,25 Все начинается с функции toggleAccordion, он меняет состояние activeIndex, Заголовок каждого элемента аккордеона (accordion__header) содержит обработчик клика onClick, который вызывает функцию toggleAccordion с индексом текущего элемента.  */}
                    <p className={`${styles.accordion__title} ${activeTitleClass}`}>{item.title}</p>
                    <img className={`${styles.icon__arrow} ${activeArrowClass}`} src={arrowSvg} alt="arrow" /> {/*Иконка стрелки (icon__arrow) поворачивается на 180 градусов, когда элемент аккордеона раскрыт */}
                  </div>
                   {/*  В JS выражение a && b работает так: если a истинно, то возвращается b. Если a ложно, то возвращается a. Если isActive истинно (то есть текущий элем. аккордеона активен), то будет отрисован блок <div className="accordion__body">...</div> */}
                  {isActive && (
